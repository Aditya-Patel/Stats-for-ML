...
def GAN_generator(gmm_samples, input_size = 100, output_size = 2, num_samples = 1600, bts=64, epoch_ct=10000, lr=0.001):
  ...
  # lr_sched_G = optim.lr_scheduler.ReduceLROnPlateau(optim_G, 'min', factor=0.1, patience=10, threshold=0.0000001)
  optim_D = optim.Adam(disc.parameters(), lr=lr)
  # lr_sched_D = optim.lr_scheduler.ReduceLROnPlateau(optim_G, 'min', factor=0.1, patience=10, threshold=0.0000001)
  ...
  for epoch in range(epoch_ct):
    for n, (real_samples, _) in enumerate(train_loader):
        ...
        loss_gen.backward()
        optim_G.step()
        # lr_sched_G.step(loss_gen)

    # Print loss
    if epoch % 100 == 0:
      test = Variable(torch.randn(num_samples, input_size))
      genr_samples = genr(test)
      mmd_score = MMD(genr_samples, torch.from_numpy(gmm_samples).to(dtype=torch.float32), kernel='multiscale')
      print(f"Epoch [{epoch}/{epoch_ct}]: D Loss: {loss_disc:.4f} | G Loss: {loss_gen:.4f} | MMD: {mmd_score:.4f}")
...
# Driver function to tie it all together
def driver(num_components, samples, stdev, input_size, lr, bts, epoch_ct):
...
if __name__ == '__main__':
  driver(COMPONENT_CT, SAMPLE_CT, STD_DEV, INPUT_SZ, LEARNING_RATE, BATCH_SZ, EPOCH_CT)